...
<script>
    import Loader from './LoaderIndex.vue';
    import { galleryServices } from '../../store//gallery/services'
                ...
                for(var i=0; i < files.length; i++){
                    if(!this.max || this.allMedia.length < this.max){
                        if(files[i].size <= this.maxFilesize*1000000){
                            ...
                            formData.set('gallery_id', this.galleryId)
    
                            // const {data} = await axios.post(this.server, formData)
                            const data = await galleryServices.insertImage(formData)
